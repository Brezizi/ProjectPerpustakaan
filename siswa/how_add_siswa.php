<?php
    ...
    if ($_POST) {
        ...
        if (empty($nisn_siswa)) {
            ...
            $insert=pg_query($conn, "insert into siswa (nisn_siswa, usn_siswa, pass_siswa, nama_siswa, jk_siswa)");

            if ($insert) {
                echo "<script>alert('Success adding student data'); location.href='add_siswa.php';</script>";
            } else {
                echo "<script>alert('Failed adding student data'); location.href='add_siswa.php';</script>";
            }
        }
    }
?>